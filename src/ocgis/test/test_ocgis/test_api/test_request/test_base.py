...
class TestRequestDataset(TestBase):
    ...
    def test_init_variable(self):
        ...
        self.assertEqual(rd._alias, ('tas',))
    ...
    def test_init_multiple_variable(self):
        ...
        self.assertEqual(rd._alias, ('tas', 'tasmax'))
    ...
    def test_init_combinations(self):
        ...
        keywords = dict(
            ...
            conform_units_to=[None, [None, None], ['celsius', 'fahrenheit'], 'crap', [None, 'kelvin'], ['crap', 'crappy'], [None, 'coulomb'], ['coulomb', 'coulomb']])
        ...
        for k in itr_products_keywords(keywords):
            try:
                ...
                self.assertEqual(len(rd._variable), len(rd._units))
                if k['name'] is None:
                    self.assertEqual(rd.name, '_'.join(rd._alias))
                else:
                    self.assertEqual(rd.name, 'foo')
                ...
                try:
                    ...
                    self.assertEqual(set(field.variables.keys()), set(rd._alias))
                except VariableNotFoundError:
                    if 'crap' in rd._variable:
                        continue
                    else:
    ...
    def test_with_bad_units_attempting_conform(self):
        ## pass bad units to the init and an attempt a conform. values from
    ...
    def test_inspect_as_dct(self):
        ...
        for variable in variables:
            ...
            self.assertEqual(ref.keys(), ['Start Date', 'End Date', 'Calendar', 'Units', 'Resolution (Days)', 'Count', 'Has Bounds', 'Spatial Reference', 'Proj4 String', 'Extent', 'Interface Type', 'Resolution'])
