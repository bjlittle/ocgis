...
#        ocgis.env.VERBOSE = True
        ...
        for c in calc:
            ...
            if c is None:
                with fiona.open(ret) as f:
                    ...
                    record_coordinates = [[(-105.5, 37.5), (-105.5, 38.5), (-104.5, 38.5), (-104.5, 37.5), (-105.5, 37.5)]]
    ...
    def test_shp_conversion_with_external_geometries(self):
        ...
        properties = OrderedDict(
            [(u'DID', 1), (u'VID', 1), (u'UGID', 2), (u'TID', 26), (u'LID', 1), (u'GID', 16.0), (u'VARIABLE', u'foo'),
             (u'ALIAS', u'foo'), (u'TIME', '2000-03-26 12:00:00'), (u'YEAR', 2000), (u'MONTH', 3), (u'DAY', 26), (u'LEVEL', 50),
             (u'VALUE', 4.0)])
        self.assertDictEqual(properties, rows[325]['properties'])
        self.assertEqual(rows[325], {'geometry': {'type': 'Polygon', 'coordinates': [[(-102.5, 39.5), (-102.5, 40.5), (-101.5, 40.5), (-101.5, 39.5), (-102.5, 39.5)]]}, 'type': 'Feature', 'id': '325', 'properties': properties})
    ...
    def test_csv_plus_conversion(self):
        ...
        with fiona.open(ugid_path) as f:
            ...
            fiona_crs = fiona_meta.pop('crs')
            self.assertEqual(CoordinateReferenceSystem(value=fiona_crs), WGS84())
            self.assertEqual(fiona_meta,{'driver': u'ESRI Shapefile', 'schema': {'geometry': 'Polygon', 'properties': OrderedDict([(u'UGID', 'int:10')])}})
